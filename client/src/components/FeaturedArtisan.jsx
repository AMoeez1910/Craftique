  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        const sortedArtisan = response.data.sort((a, b) => b.itemsSold - a.itemsSold);
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
          {
            artisan.slice(0, 4).map((item) => (
              <div className="relative group grid [grid-template-areas:stack] overflow-hidden rounded-lg" key={item.brand._id} onClick={() => {
                ...
                <div className="flex-1 [grid-area:stack] bg-gradient-to-t from-black/70 to-transparent group-hover:opacity-90 transition-opacity text-white p-6 justify-end flex flex-col gap-2">
                  <h3 className="font-semibold text-lg tracking-tight">{item.brand.name}</h3>
                </div>
              </div>
            ))
          }
